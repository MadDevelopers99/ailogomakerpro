...
const FIELD_LABELS = {
  name: "Full Name", job_title: "Job Title", company: "Company Name", phone: "Phone", email: "Email", website: "Website",
  tagline: "Tagline", logo_mark: "Logo Letter", address: "Address", footer_text: "Footer Text",
  headline: "Headline", subheadline: "Subheadline", date_location: "Date / Location",
  detail_1: "Detail Line 1", detail_2: "Detail Line 2", detail_3: "Detail Line 3", cta_text: "Call To Action",
  invoice_label: "Document Title", invoice_number: "Invoice Number", invoice_date: "Invoice Date",
  bill_to_name: "Bill To (Name)", bill_to_address: "Bill To (Address)",
  item_1: "Item 1", item_1_price: "Item 1 Price", item_2: "Item 2", item_2_price: "Item 2 Price",
  item_3: "Item 3", item_3_price: "Item 3 Price", total_label: "Total Label", total_value: "Total Amount", footer_note: "Footer Note",
  restaurant_name: "Restaurant Name", category_1_title: "Category 1", item_1_1: "Item 1.1", item_1_2: "Item 1.2",
  category_2_title: "Category 2", item_2_1: "Item 2.1", item_2_2: "Item 2.2", category_3_title: "Category 3", item_3_1: "Item 3.1",
  greeting: "Greeting", subtext: "Subtext", amount: "Amount", signature_label: "Signature Label", date_label: "Date Label",
  event_name: "Event Name", date_time: "Date & Time", location: "Location", rsvp: "RSVP Info",
  heading: "Heading", message: "Message", signature: "Signature", line_1: "Line 1", line_2: "Line 2",
};
...
function applyCanvasSize() {
  const w = project.canvas?.width || 850;
  const h = project.canvas?.height || 1100;
  canvas.width = w;
  canvas.height = h;
...
function addTextElement() {
  ...
  commitAction((p) => {
    const w = p.canvas?.width || 850;
    const h = p.canvas?.height || 1100;
    p.elements.push({
      id, type: "text", text: "New Text", label: "Custom Text",
      x: Math.round(w / 2 - 60), y: Math.round(h / 2),
      fontSize: Math.max(12, Math.round(Math.min(w, h) * 0.06)), fontFamily: "Poppins", color: "#000000", align: "left", bold: false,
...
function logoPickerHtml() {
  ...
  const hasLogoCircle = !!elementById("logo_circle");
  if (!hasLogoCircle) return "";
  return `
    <div class="field">
      <label>Logo</label>
...
document.getElementById("downloadBtn").addEventListener("click", () => {
  const nameSource = ["company", "restaurant_name", "event_name", "name", "headline"].map((id) => elementById(id)?.text).find(Boolean) || project.categorySlug || "design";
  const name = nameSource.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
...
async function init() {
  const printCat = qs("cat");
  const [printData, catData, logoData, bgData, meta] = await Promise.all([
    printCat ? fetch(`data/print-templates/${printCat}.json`).then((r) => r.json()) : Promise.resolve(null),
    fetch("data/categories.json").then((r) => r.json()),
    fetch("data/logos.json").then((r) => r.json()),
    fetch("data/all-backgrounds.json").then((r) => r.json()),
    fetch("data/print-templates-meta.json").then((r) => r.json()),
  ]);
  categories = catData;
  logos = logoData;
  pickerCat = categories[0]?.id || null;
  printBackgrounds = bgData;
  bgThemes = Object.keys(bgData).sort();
  bgPickerTheme = bgThemes[0] || null;

  const templateId = qs("id");
  let template = printData?.templates.find((t) => t.id === templateId);
  if (!template) {
    for (const c of meta.categories) {
      const list = await fetch(`data/print-templates/${c.slug}.json`).then((r) => r.json());
  ...
  if (!template) {
    ...
    const list = await fetch(`data/print-templates/${first.slug}.json`).then((r) => r.json());
    template = list.templates[0];
  }

  project = getCurrentPrintProject();
  if (!project || project.sourceTemplateId !== template.id) {
    project = startPrintProject(template);
  }
