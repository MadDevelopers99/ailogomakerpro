import { startMotionProject, getCurrentMotionProject, setCurrentMotionProject, saveCurrentMotion } from "./motion-store.js";
import { renderCardToCanvas } from "./render-card.js";
import { recordCanvasAnimation, isRecordingSupported } from "./motion-recorder.js";
...
const FIELD_LABELS = {
  headline: "Headline", subheadline: "Subheadline", handle: "Handle / Website", logo_mark: "Logo Letter",
};
...
let currentTime = 0; // ms; equals project.durationMs when "settled" (edit-ready)
let isPlaying = false;
...
function drawSelectionHandle(ctx) {
  if (!selectedId || currentTime !== project.durationMs) return;
...
async function draw() {
  applyCanvasSize();
  const { ctx, boxes } = await renderCardToCanvas(canvas, project, { selectedId, time: currentTime });
  lastBoxes = boxes;
...
function updateTransportUI() {
  const dur = project.durationMs || 2000;
  scrubber.max = dur;
  scrubber.value = currentTime;
  timeLabel.textContent = `${(currentTime / 1000).toFixed(1)}s / ${(dur / 1000).toFixed(1)}s`;
}

function selectField(id) {
  if (currentTime !== project.durationMs) return;
  selectedId = id;
  draw();
...
function applyLogoImage(src) {
  commitAction((p) => {
    ...
    if (!img) {
      img = { id: "logo_image", type: "image", x: circle?.x ?? 0, y: circle?.y ?? 0, width: circle?.width ?? 90, height: circle?.height ?? 90, rotation: 0, opacity: 1, src, visible: true, editable: true, animation: circle?.animation };
      p.elements.push(img);
    } else {
      img.src = src;
      img.visible = true;
...
function renderPanel() {
  ...
  rightPanel.innerHTML = `
    <h3>Edit ${project.category || "Animation"}</h3>
    ${logoPickerHtml()}
    ${fields.map(textFieldHtml).join("")}
    ${backgroundPanelHtml(project.background, { bgCategories: bgThemes })}
    <div style="font-size:11.5px;color:var(--text-faint);margin-top:6px;">✎ Drag text to reposition · drag the purple handle to resize · click to select. Editing works at the settled (end) frame — press Preview to see the motion.</div>
  `;
  ...
  wireBackgroundPanel(rightPanel, () => project.background, {
    ...
    cardBackgrounds: socialBackgrounds,
  });
...
canvas.addEventListener("pointerdown", (e) => {
  e.preventDefault();
  if (isPlaying || isRecording || currentTime !== project.durationMs) return;
...
async function playPreview() {
  ...
  await new Promise((resolve) => {
    function tick(now) {
      const elapsed = Math.min(dur, now - start);
      currentTime = elapsed;
      draw();
...
async function recordAndDownload() {
  ...
  if (!isRecordingSupported()) {
    alert("Recording isn't supported in this browser. Try the latest Chrome or Edge.");
    return;
  }
  isRecording = true;
  const originalLabel = recordBtn.textContent;
  recordBtn.textContent = "⬤ Recording…";
  recordBtn.disabled = true;
  playBtn.disabled = true;
  try {
    const dur = project.durationMs || 2000;
    const blob = await recordCanvasAnimation(canvas, {
      fps: 30,
      durationMs: dur,
      onFrame: (elapsed) => { currentTime = elapsed; draw(); },
    });
    const brand = elementById("headline")?.text || project.category || "animation";
    const name = brand.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "animation";
  ...
  isRecording = false;
  recordBtn.textContent = originalLabel;
  recordBtn.disabled = false;
  playBtn.disabled = false;
  currentTime = project.durationMs || 2000;
  await draw();
}
recordBtn.addEventListener("click", recordAndDownload);

async function init() {
  const cat = qs("cat");
  const [motionData, catData, logoData, bgData, meta] = await Promise.all([
    cat ? fetch(`data/motion-templates/${cat}.json`).then((r) => r.json()) : Promise.resolve(null),
    fetch("data/categories.json").then((r) => r.json()),
    fetch("data/logos.json").then((r) => r.json()),
    fetch("data/social-backgrounds.json").then((r) => r.json()),
    fetch("data/motion-templates-meta.json").then((r) => r.json()),
  ]);
  categories = catData;
  logos = logoData;
  ...
  let template = motionData?.templates.find((t) => t.id === templateId);
  if (!template) {
    for (const c of meta.categories) {
      const list = await fetch(`data/motion-templates/${c.slug}.json`).then((r) => r.json());
  ...
  if (!template) {
    ...
    const list = await fetch(`data/motion-templates/${first.slug}.json`).then((r) => r.json());
    template = list.templates[0];
  }

  project = getCurrentMotionProject();
  if (!project || project.sourceTemplateId !== template.id) {
    project = startMotionProject(template);
  }
  currentTime = project.durationMs || 2000;
