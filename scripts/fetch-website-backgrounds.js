// One-time downloader for the 3 new imagery themes the Website Builder needs
// that aren't already covered by assets/business-backgrounds or
// assets/social-backgrounds (business, restaurant, abstract/minimal are reused
// as-is). Saves under assets/website-backgrounds/<theme>/.
//
// Usage: node --env-file=.env scripts/fetch-website-backgrounds.js
const fs = require("fs");
...
if (!API_KEY) {
  console.error("Missing PEXELS_API_KEY. Run with: node --env-file=.env scripts/fetch-website-backgrounds.js");
  process.exit(1);
}

const ROOT = path.join(__dirname, "..");
const OUT_DIR = path.join(ROOT, "assets", "website-backgrounds");

const THEME_QUERIES = {
  portfolio: ["creative designer desk background", "art studio workspace background"],
  "online-store": ["product photography flatlay background", "retail shopping bags background"],
  personal: ["cozy home office background", "minimal home interior background"],
};
